...
Public Function PuRangeCopy(strFile As String, strBook As String, strRange As String) As Boolean
    ...
    With Application
        ...
        ' Clear the target sheet
        shtBerTemp.Range("A1").CurrentRegion.Clear
        FileSystem.SetAttr strFile, Attributes:=vbNormal
        .Workbooks.Open strFile, True
        ...
        Range(strRange).CurrentRegion.Select
        Selection.Copy
        shtBerTemp.Range("A1").PasteSpecial xlPasteValues
        Workbooks(strBook).Close
...
Public Function bPuBedOpslaan(strFileName As String, sCONST_WORKBOOKNAME As String, strTekstFile, strTekstBookName) As Boolean
...
'    On Error GoTo ErrFileSave
    ...
    If PuPatientSave Then
        strSelectie = "a1:B" + CStr(shtPatData.Range("b1").CurrentRegion.Rows.Count)
        strAfsprakenTekst = "a1:c" + CStr(shtPatAfsprakenTekst.Range("c1").CurrentRegion.Rows.Count)
        ...
        With Workbooks(strTekstBookName)
            ...
            shtPatAfsprakenTekst.Range(strAfsprakenTekst).Copy
...
Public Function oPuPatientenCollectie() As Collection

    Dim colPatienten As New Collection
    Dim bed As String, vn As String, an As String, geb As String
    
    If PuRangeCopy(GetPatientDataPath() + "Patienten.xls", "Patienten.xls", "a1") Then
        With colPatienten
            For intCount = 2 To shtBerTemp.Range("A1").CurrentRegion.Rows.Count
                With shtBerTemp
    ...
    Set oPuPatientenCollectie = colPatienten
    Set colPatienten = Nothing

End Function

Public Function oPuRowsDelete(oSheet As Worksheet, sKolom As String) As Worksheet
